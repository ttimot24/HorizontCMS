...
<div class='container'>
	...
	@if($slider_images->count() > 0)
		@foreach($slider_images as $image)
			...
			<a class='btn btn-danger btn-xs btn-block' href='admin/headerimages/getoffslider/{{ $image->id }}'>Remove from slider</a>
			</div>
		@endforeach
	@else
...
@foreach($dirs as $each)
	...
	<a class='btn-sm btn-success col-md-6' href='admin/headerimages/addtoslider/{{ $each }}'>Add to slider</a>
	<a href='admin/headerimages/delete/{{ $each }}' class='pull-right'>
	...
	<img src='images/header_images/{{ $each }}' alt='' class='img-rounded' width='100%' height='80%;'>
	</div>
@endforeach
