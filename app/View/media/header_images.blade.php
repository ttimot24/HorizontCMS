...
@section('content')
                ...
                @if ($slider_images->count() > 0)
                    @foreach ($slider_images as $each)
                        <div class="card col-md-3 pt-3">
                            ...
                            <img src='storage/images/header_images/{{ $each->image }}' alt='' class='card-img-top'
                            ...
                            <ul class="list-group list-group-flush mb-3">
                                <a class='btn btn-danger btn-xs btn-block'
                                    href='admin/header-image/remove-from-slider/{{ $each->id }}'>Remove from slider</a>
                            </ul>
                        </div>


                        <div class='modal edit_images' id='headline-image-{{ $each->id }}' tabindex='-1' role='dialog'
                ...
                @foreach ($slider_disabled as $each)
                    <div class="card col-md-3">
                        <div class="card-header py-2 px-0 bg-white">
                            <a class='btn-sm btn-success col-md-6'
                                href='admin/header-image/add-to-slider/{{ $each->id }}'>Add to slider</a>
                            <a href='admin/header-image/delete/{{ $each->id }}' class='pull-right'>
                                <span class='fa fa-trash' aria-hidden='true'
                                    style=' font-size: 1.4em;z-index:15;top:3px;right:3px;margin-bottom:-15px;'></span>
                            </a>
                            <a class='pull-right me-2' data-bs-toggle='modal'
                                data-bs-target='#headline-image-{{ $each->id }}'>
                                <span class='fa fa-pencil' aria-hidden='true'
                                    style=' font-size: 1.4em;z-index:15;top:3px;right:3px;margin-bottom:-15px;'></span>
                            </a>
    ...
    <div class='modal upload_images' id='create_file' tabindex='-1' role='dialog' aria-labelledby='myModalLabel'
        aria-hidden='true'>
        <div class='modal-dialog modal-xl'>
            <div class='modal-content'>
                <div class='modal-header modal-header-primary bg-primary'>
