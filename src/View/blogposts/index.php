...
<h2>Posted news <small class='pull-right' style='margin-top:1.5%;'>All: <?php echo $data['number']; ?></small></h2></br>
<div class='container'>
...
<?php foreach($data['all'] as $blogpost): ?>

<tr>
  <td><?= $blogpost->id ?></td>


  <td><img src=<?= $blogpost->get_thumb(); ?>  class='img img-rounded' style='object-fit:cover;' width=70 height=50 /> </td>


  <td  class='col-md-5'><a href=admin/blogpost/view/<?= $blogpost->id; ?> ><?= $blogpost->title; ?></a></td>
  		 <td><center><span class="badge" style='font-size:14px'><?= count($blogpost->get_comments()) ?></span></center></td>
  

  <td><?= date('Y-m-d',$blogpost->date); ?></br><font size='2'><i>at</i> <?= date('H:i:s',$blogpost->date) ?></font></td>
         <td><a href=admin/user/view/<?= $blogpost->get_author()->id ?> >
         <?= $blogpost->get_author()->username ?></a></td>
         <td><span class="label label-success" style='font-size:14px; display:block'><?= $blogpost->get_category()->name ?></span></td>
